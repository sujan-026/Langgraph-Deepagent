...
# Required for Bedrock model usage
...
Before running the project, log into AWS SSO for that profile:

```bash
aws sso login --profile your-aws-sso-profile
```

The Bedrock client uses the normal AWS credential chain, so `AWS_PROFILE` is enough
when your SSO session is active.

If Bedrock returns a validation error saying on-demand throughput is not supported,
set `BEDROCK_MODEL_ID` to the inference profile ID or ARN for that model instead of
